...
///   destroys its Rigidbody (eliminating all jitter when the player walks), and parents
///   it as a static child of the socket's attach point so it moves exactly with the Bottle.
///
/// Inspector wiring:
...
public class CorkSnapZone : MonoBehaviour
{
    ...
    private IEnumerator StabiliseCork(Transform corkTf)
    {
        ...
        // Wait for XRI to process the exit event before we destroy the Rigidbody.
        yield return null;

        // Disable grab so the Cork can never be picked up again.
        if (interactable != null) interactable.enabled = false;

        // Destroy the Rigidbody entirely — Cork becomes a pure Transform,
        // no physics simulation, no interpolation, no drift.
